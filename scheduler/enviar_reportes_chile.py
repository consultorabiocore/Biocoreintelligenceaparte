...
class AutomaticReportScheduler:
    ...
    def obtener_clientes_a_reportar(self):
        ...
        try:
            ...
            clientes_a_reportar = []
            hoy = datetime.now(TIMEZONE_CHILE)
    ...
    def _verificar_horario(self, hora_reporte):
        ...
        try:
            ...
            ahora_chile = datetime.now(TIMEZONE_CHILE)
            hora_actual = ahora_chile.time()
            
            # Dar margen de 1 hora (entre la hora programada y 1 hora después)
            inicio = hora_programada
            fin = (datetime.combine(ahora_chile.date(), hora_programada) + timedelta(hours=1)).time()
    ...
    def _es_dia_reporte_semanal(self, cliente):
        ...
        try:
            ...
            hoy = datetime.now(TIMEZONE_CHILE).date()
    ...
    def _procesar_cliente(self, cliente):
        ...
        try:
            ...
            if exito:
                ...
                self.supabase.actualizar_ultimo_envio(cliente.get('id'), datetime.now(TIMEZONE_CHILE))
