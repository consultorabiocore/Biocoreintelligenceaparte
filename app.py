...
def _insar_default_status():
    return {
        ...
        "interpretation_scope": (
            ...
            "desenvolvimiento de fase y geocodificación. BioCore Intelligence no sustituye ese flujo con GRD."
        ),
    }
...
def crear_portada_biocore():
    ...
    # -----------------------------------------------------------------------
    logo_hd = Path("logo_biocore_hd.png")
    if logo_hd.exists():
        st.image(str(logo_hd), width=76)
    ...
    .bc-wordmark{display:flex;align-items:baseline;gap:.24rem;font-size:1.05rem;font-weight:800;letter-spacing:-.02em;color:#f8fafc;margin-bottom:16px;}
    .bc-wordmark .core{color:#84cc16;}
    .bc-wordmark .intel{color:#94a3b8;font-size:.78rem;font-weight:650;letter-spacing:.08em;text-transform:uppercase;margin-left:.2rem;}
    .bc-brand{
        color:#7dd3fc;
        font-size:.73rem;
        font-weight:800;
    ...
    <div class="bc-home">
      <section class="bc-hero">
        <div class="bc-wordmark"><span>Bio</span><span class="core">Core</span><span class="intel">Intelligence</span></div>
        <div class="bc-brand">Vigilancia ambiental inteligente</div>
    ...
    """)
    if logo_hd.exists():
        st.html("<style>.bc-wordmark{display:none !important;}</style>")
...
def build_temporal_bundle(
    ...
    if spec.mode == "scene":
        ...
        if len(spectral) < expected_min:
            n_valid = len(spectral)
            warnings.append(
                f"Serie temporal de {spec.label.lower()}: {n_valid} adquisición(es) óptica(s) válida(s). "
                "No se estima una tendencia con menos de dos observaciones; el estado técnico principal "
                "usa la comparación adaptativa y puede seguir siendo evaluable."
            )
...
def build_surveillance_diagnosis(
    ...
    if triggered:
        ...
        title = (
            ...
            else "Sin alerta prioritaria en la evaluación actual"
        )
        recommendations = [
...
def evaluate_monitoring_plan(
    ...
    if not plan:
        return {
            ...
            "summary": (
                "BioCore Intelligence todavía no tiene cargadas las obligaciones y umbrales específicos de este proyecto. "
                "Esto no significa que el proyecto no tenga una RCA, permisos o planes de seguimiento; significa que esas reglas "
                "aún no se han incorporado a la plataforma para compararlas automáticamente con los datos."
            ),
        }
...
BASE_REFERENCES = [
    {
        ...
        "role": (
            ...
            "regeneracion, condición, magnitud, duración, extensión, biodiversidad, "
    ...
    },
    {
        ...
        "role": "Reflectancia superficial e índices espectrales recientes.",
    },
    {
        "id": "LANDSAT-C2-L2",
        "citation": "USGS Landsat Collection 2 Level-2 Surface Reflectance.",
        "role": "Comparación temporal homogénea cuando la linea base precede a Sentinel-2 SR.",
    },
    {
        "id": "MOD11A1-061",
...
HUMEDAL_REFERENCE = {
    ...
    "role": (
        "Sensores remotos como apoyo de gabinete; limites y condiciónes deben "
        "confirmarse/ajustarse con criterios de hidrologia, vegetación y suelos "
        "cuando corresponda."
    ),
}
...
def build_canonical_report(
    ...
    metrics = [
        {
            "code": "NDVI",
            "variable": "Indice de vegetación de diferencia normalizada",
            "unit": "adimensional",
            "current": current.get("ndvi"),
            "baseline": baseline.get("ndvi"),
            "delta": (changes.get("ndvi") or {}).get("delta"),
            "pct": (changes.get("ndvi") or {}).get("pct"),
            "interpretation_scope": "Verdor/cobertura fotosintetica; no prueba degradación por si solo.",
        },
        {
            "code": "SAVI",
            "variable": "Indice de vegetación ajustado al suelo",
            "unit": "adimensional",
            "current": current.get("savi"),
            "baseline": baseline.get("savi"),
            "delta": (changes.get("savi") or {}).get("delta"),
            "pct": (changes.get("savi") or {}).get("pct"),
            "interpretation_scope": "Respuesta de vegetación reduciendo influencia del brillo del suelo.",
        },
        {
            "code": "NDWI",
            "variable": "Indice Green-NIR para señal de agua superficial",
            "unit": "adimensional",
            "current": current.get("ndwi"),
            "baseline": baseline.get("ndwi"),
            "delta": (changes.get("ndwi") or {}).get("delta"),
            "pct": (changes.get("ndwi") or {}).get("pct"),
            "interpretation_scope": "Señal espectral de agua superficial; no identifica origen ni causalidad.",
        },
        {
            "code": "NDMI",
            "variable": "Indice NIR-SWIR1 para señal de humedad",
            "unit": "adimensional",
            "current": current.get("ndmi"),
            "baseline": baseline.get("ndmi"),
            "delta": (changes.get("ndmi") or {}).get("delta"),
            "pct": (changes.get("ndmi") or {}).get("pct"),
            "interpretation_scope": "Señal de humedad; no equivale a agua acumulada.",
        },
        {
            "code": "NDSI",
            "variable": "Indice Green-SWIR1 para señal de nieve/hielo",
            "unit": "adimensional",
        ...
        {
            "code": "SWIR1/SWIR2",
            "variable": "Razón espectral SWIR1/SWIR2",
            "unit": "adimensional",
            ...
            "interpretation_scope": "Razón espectral; no se denomina contenido de arcilla sin calibracion.",
        },
    ]
    ...
    report = {
        ...
        "scope": {
            ...
            "purpose": (
                ...
                "teledetección trazable. No sustituye evaluación ambiental, "
...
def _metric_from_report(report: Dict[str, Any], code: str) -> Dict[str, Any]:
    for metric in report.get("metrics") or []:
        if str(metric.get("code") or "").upper() == code.upper():
            return metric
    return {}


def _plain_metric_change(report: Dict[str, Any], code: str, decimals: int = 3) -> str:
    m = _metric_from_report(report, code)
    current = _num(m.get("current"))
    baseline = _num(m.get("baseline"))
    if current is None:
        return "Sin dato válido"
    if baseline is None:
        return f"{current:.{decimals}f} (sin referencia comparable)"
    direction = "aumentó" if current > baseline else "disminuyó" if current < baseline else "se mantuvo"
    return f"{direction} de {baseline:.{decimals}f} a {current:.{decimals}f}"


def format_telegram(report: Dict[str, Any]) -> str:
    """Resumen breve para clientes; el detalle técnico queda en los PDF."""
    validate_canonical_report(report)
    ...
    project_name = project.get("name") or "Proyecto"
    level = str(diagnosis.get("level") or "N/D")
    level_human = {
        "SIN SEÑAL PRIORITARIA": "Sin alerta prioritaria",
        "SEÑAL A REVISAR": "Señal a revisar",
        "VERIFICACIÓN PRIORITARIA": "Verificación prioritaria",
        "EVIDENCIA INSUFICIENTE": "Datos insuficientes",
    }.get(level.upper(), level.title())

    snow = _num(context.get("scl_snow_ice_fraction"))
    water = _num(context.get("scl_water_fraction"))
    vegetation = _num(context.get("scl_vegetation_fraction"))
    lst = _num(context.get("lst_mean_c"))

    lines = [
        "🛰️ BIOCORE INTELLIGENCE",
        f"📍 {project_name} · {level_human}",
        "",
    ]

    if snow is not None:
        lines.append(
            f"❄️ Nieve/hielo: {snow:.0%} de la escena. "
            f"NDSI (indicador de nieve/hielo): {_plain_metric_change(report, 'NDSI')}."
        )

    if _metric_from_report(report, "NDMI"):
        note = f"💧 Humedad: NDMI {_plain_metric_change(report, 'NDMI')}"
        if (snow or 0) > 0.05:
            note += "; la nieve/hielo puede influir en esta señal"
        lines.append(note + ".")

    if _metric_from_report(report, "NDWI"):
        water_text = f"; clasificación de agua: {water:.0%}" if water is not None else ""
        lines.append(
            f"🌊 Agua superficial: NDWI {_plain_metric_change(report, 'NDWI')}{water_text}."
        )

    if vegetation is not None:
        if vegetation < 0.10 and (snow or 0) > 0.50:
            lines.append(
                f"🌱 Vegetación: {vegetation:.0%} de la escena; con esta cobertura de nieve/hielo "
                "no se interpreta NDVI/SAVI como pérdida de vegetación."
            )
        else:
            lines.append(
                f"🌱 Vegetación: {vegetation:.0%} de la escena; NDVI {_plain_metric_change(report, 'NDVI')}."
            )

    climate_bits = []
    if lst is not None:
        climate_bits.append(f"superficie {lst:.1f} °C")
    fire = ancillary.get("active_fire") or {}
    if fire.get("available"):
        nfire = int(fire.get("combined_nominal_high_pixel_days") or 0)
        climate_bits.append("sin anomalías térmicas VIIRS" if nfire == 0 else f"{nfire} detecciones térmicas VIIRS")
    if climate_bits:
        lines.append("🌡️ " + " · ".join(climate_bits) + ".")

    n_obs = int(temporal.get("n_spectral_records") or 0)
    range_label = str(temporal.get("range_label") or "período seleccionado").lower()
    if n_obs < 2:
        lines += [
            "",
            f"⚠️ {range_label.capitalize()}: {n_obs} observación óptica válida; aún no se confirma una tendencia temporal.",
        ]

    recs = diagnosis.get("recommendations") or []
    if recs:
        lines.append(f"➡️ Próximo paso: {recs[0]}")

    lines += [
        "",
        "📄 El informe ejecutivo explica los hallazgos; el informe técnico contiene metodología y trazabilidad.",
        f"🔐 ID técnico: {_short_id(report)}",
    ]
    return "\n".join(lines)
...
class BioCorePDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 5, _clean_pdf_text("BIOCORE INTELLIGENCE - VIGILANCIA AMBIENTAL"))
        self.set_xy(12, 13.5)
        self.set_font("helvetica", "", 7.3)
        self.cell(0, 4, _clean_pdf_text(f"ID técnico: {_short_id(self.report)}"))
        self.ln(14)
    ...
    def footer(self):
        ...
        self.cell(
            0, 4,
            _clean_pdf_text(
                f"BioCore Intelligence | {_short_id(self.report)} | Página {self.page_no()}/{{nb}}"
            ),
            align="C",
        )
...
def _pdf_signal_box(pdf: FPDF, title: str, text: str):
    pdf.set_fill_color(245, 248, 251)
    pdf.set_draw_color(210, 219, 228)
    x, y = pdf.get_x(), pdf.get_y()
    pdf.set_font("helvetica", "B", 9)
    title_h = 5
    body_lines = max(2, (len(str(text)) // 92) + 1)
    h = 9 + body_lines * 4.2
    pdf.rect(x, y, 190, h, "DF")
    pdf.set_xy(x + 4, y + 3)
    pdf.set_text_color(23, 55, 82)
    pdf.cell(182, title_h, _clean_pdf_text(title))
    pdf.set_xy(x + 4, y + 8)
    pdf.set_font("helvetica", "", 8.1)
    pdf.set_text_color(35, 35, 35)
    pdf.multi_cell(182, 4.2, _clean_pdf_text(text), new_x="LMARGIN", new_y="NEXT")
    pdf.set_y(y + h + 3)


def _client_observations(report: Dict[str, Any]) -> List[Tuple[str, str]]:
    context = report.get("scene_context") or {}
    ancillary = report.get("ancillary") or {}
    temporal = report.get("temporal") or {}
    ...
    if snow is not None:
        items.append((
            "Nieve y hielo",
            f"La escena reciente está clasificada en {snow:.0%} como nieve/hielo. "
            f"El NDSI, indicador de nieve/hielo, {_plain_metric_change(report, 'NDSI')}."
        ))

    if _metric_from_report(report, "NDMI"):
        note = f"El NDMI, indicador de humedad, {_plain_metric_change(report, 'NDMI')}."
        if (snow or 0) > 0.05:
            note += " La cobertura de nieve/hielo puede contribuir a esta señal, por lo que se interpreta junto con el contexto de escena."
        items.append(("Humedad", note))

    if _metric_from_report(report, "NDWI"):
        note = f"El NDWI, indicador espectral de agua superficial, {_plain_metric_change(report, 'NDWI')}."
        if water is not None:
            note += f" La clasificación reciente identifica {water:.0%} de agua superficial."
        items.append(("Agua superficial", note))

    if vegetation is not None:
        note = f"La escena reciente registra {vegetation:.0%} de vegetación según SCL."
        if vegetation < 0.10 and (snow or 0) > 0.50:
            note += " Debido al dominio de nieve/hielo, los índices de vegetación no se usan en esta ejecución para afirmar pérdida o estrés vegetal."
        else:
            note += f" NDVI: {_plain_metric_change(report, 'NDVI')}; SAVI: {_plain_metric_change(report, 'SAVI')}."
        items.append(("Vegetación", note))

    if lst is not None:
        items.append(("Temperatura de superficie", f"MODIS registró una temperatura media de superficie de {lst:.1f} °C en la ventana disponible."))

    fire = ancillary.get("active_fire") or {}
    if fire.get("available"):
        n = int(fire.get("combined_nominal_high_pixel_days") or 0)
        items.append((
            "Anomalías térmicas",
            "No se detectaron anomalías térmicas VIIRS en la ventana reciente." if n == 0
            else f"Se registraron {n} detecciones térmicas píxel-día VIIRS en la ventana reciente."
        ))

    if int(temporal.get("n_spectral_records") or 0) < 2:
        items.append((
            "Serie temporal",
            f"Para {str(temporal.get('range_label') or 'el período seleccionado').lower()} existe "
            f"{int(temporal.get('n_spectral_records') or 0)} observación óptica válida. "
            "Aún no hay suficientes fechas para confirmar una tendencia temporal."
        ))
...
def render_pdf(
    ...
    """PDF orientado al cliente: primero hallazgos y valor; metodología al final."""
    validate_canonical_report(report)
    ...
    pdf.multi_cell(0, 8, _clean_pdf_text("INFORME EJECUTIVO DE VIGILANCIA AMBIENTAL"), new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(65, 65, 65)
    pdf.set_font("helvetica", "", 8.5)
    pdf.multi_cell(
        0, 4.5,
        _clean_pdf_text(
            "BioCore Intelligence integra observación satelital, comparación temporal y evidencia complementaria para mostrar qué cambió, qué significa y qué conviene revisar."
        ),
        new_x="LMARGIN", new_y="NEXT",
    )

    _pdf_section(pdf, "1. Resumen del proyecto")
    _pdf_kv(pdf, "Proyecto", project.get("name"))
    _pdf_kv(pdf, "Tipo", project.get("type"))
    _pdf_kv(pdf, "Período analizado", f"{trace.get('current_window_start') or 'N/D'} a {trace.get('current_window_end') or 'N/D'}")
    _pdf_kv(pdf, "Referencia", f"{trace.get('baseline_window_start') or 'N/D'} a {trace.get('baseline_window_end') or 'N/D'}")
    _pdf_kv(pdf, "Estado de vigilancia", diagnosis.get("level") or "N/D")
    _pdf_kv(pdf, "Generado", _pdf_date(report.get("generated_at")))

    _pdf_section(pdf, "2. Qué detectó BioCore Intelligence")
    observations = _client_observations(report)
    for title, body in observations[:6]:
        _pdf_signal_box(pdf, title, body)

    if pdf.get_y() > 225:
        pdf.add_page()

    _pdf_section(pdf, "3. Qué aporta este análisis")
    _pdf_body(
        pdf,
        "El valor del monitoreo está en comparar el mismo territorio con una referencia técnicamente compatible, conservar la trazabilidad de sensores y fechas, detectar cambios que merecen seguimiento y documentar el resultado para futuras comparaciones.",
        size=8.6,
    )
    vf = _num(quality.get("valid_fraction"))
    if vf is not None:
        _pdf_body(pdf, f"Cobertura válida del análisis comparable: {vf:.1%}.", size=8.4)
    _pdf_body(
        pdf,
        f"Sensor usado para la comparación formal: {trace.get('comparison_sensor') or 'N/D'} a {trace.get('comparison_scale_m') or 'N/D'} m.",
        size=8.4,
    )

    # Regulatory status phrased for non-specialists.
    _pdf_section(pdf, "4. Criterios específicos del proyecto")
    if regulatory.get("valid_plan"):
        _pdf_body(pdf, regulatory.get("summary") or "Criterios específicos cargados y evaluados.", size=8.4)
    else:
        _pdf_body(
            pdf,
            "Las obligaciones y umbrales propios del proyecto todavía no están cargados en BioCore Intelligence. Esto no significa que el proyecto no tenga permisos o una Resolución de Calificación Ambiental; significa que la plataforma aún no ha incorporado esas reglas para cruzarlas automáticamente con este análisis.",
            size=8.4,
        )

    _pdf_section(pdf, "5. Próximos pasos")
    recs = diagnosis.get("recommendations") or []
    for rec in recs[:3]:
        _pdf_body(pdf, f"- {rec}", size=8.3)

    # One compact scope note only.
    pdf.set_fill_color(250, 247, 235)
    pdf.set_draw_color(225, 212, 158)
    x, y = pdf.get_x(), pdf.get_y() + 2
    pdf.rect(x, y, 190, 20, "DF")
    pdf.set_xy(x + 4, y + 3)
    pdf.set_font("helvetica", "B", 8.2)
    pdf.set_text_color(70, 60, 30)
    pdf.cell(182, 4, _clean_pdf_text("Alcance del informe"))
    pdf.set_xy(x + 4, y + 8)
    pdf.set_font("helvetica", "", 7.7)
    pdf.multi_cell(
        182, 3.8,
        _clean_pdf_text(
            "La teledetección aporta evidencia objetiva y repetible para vigilancia. Cuando una conclusión exige atribuir causa, verificar una obligación específica o caracterizar un componente en terreno, BioCore Intelligence lo identifica como una etapa adicional de verificación."
        ),
        new_x="LMARGIN", new_y="NEXT",
    )

    # Evidencia técnica resumida: continúa en la misma página cuando hay espacio.
    if pdf.get_y() > 185:
        pdf.add_page()
    _pdf_section(pdf, "6. Evidencia técnica resumida")
    client_labels = {
        "NDVI": "Vegetación (NDVI)",
        "SAVI": "Vegetación ajustada al suelo (SAVI)",
        "NDWI": "Agua superficial (NDWI)",
        "NDMI": "Humedad (NDMI)",
        "NDSI": "Nieve/hielo (NDSI)",
        "SWIR1": "Reflectancia SWIR1",
        "SWIR1/SWIR2": "Relación SWIR1/SWIR2",
    }
    widths = [48, 30, 30, 30, 52]
    headers = ["Qué mide", "Actual", "Referencia", "Cambio", "Lectura"]
    pdf.set_fill_color(47, 89, 126)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("helvetica", "B", 7)
    for w, h in zip(widths, headers):
    ...
    for m in report.get("metrics") or []:
        code = m.get("code") or "N/D"
        label = client_labels.get(code, code)
        current = _fmt(m.get("current"))
        baseline = _fmt(m.get("baseline"))
        delta = "N/D" if m.get("delta") is None else f"{float(m.get('delta')):+.4f}"
        reading = {
            "NDVI": "Señal de vegetación",
            "SAVI": "Señal de vegetación con ajuste de suelo",
            "NDWI": "Señal de agua superficial",
            "NDMI": "Señal de humedad",
            "NDSI": "Señal de nieve/hielo",
            "SWIR1": "Respuesta espectral SWIR",
            "SWIR1/SWIR2": "Relación espectral SWIR",
        }.get(code, "Indicador espectral")
        row = [label, current, baseline, delta, reading]
        h = 10
        x0, y0 = pdf.get_x(), pdf.get_y()
        for idx, (w, val) in enumerate(zip(widths, row)):
            pdf.rect(x0 + sum(widths[:idx]), y0, w, h)
            pdf.set_xy(x0 + sum(widths[:idx]) + 1, y0 + 1.5)
            pdf.set_font("helvetica", "", 6.8)
            pdf.multi_cell(w - 2, 3.4, _clean_pdf_text(val), align="C")
        pdf.set_xy(x0, y0 + h)

    _pdf_section(pdf, "7. Radar, temperatura y alertas")
    ancillary = report.get("ancillary") or {}
    s1 = ancillary.get("sentinel1") or {}
    if s1.get("available"):
        blocks = []
        for key in ("ascending", "descending"):
            b = (s1.get("passes") or {}).get(key) or {}
            cur = b.get("current") or {}
            if cur.get("vv_db") is not None:
                blocks.append(f"{b.get('orbit_pass')}: VV {_fmt(cur.get('vv_db'), 2, ' dB')}")
        _pdf_body(pdf, "Sentinel-1 SAR: " + ("; ".join(blocks) if blocks else "datos disponibles") + ".", size=8.2)
        has_ref = any(((s1.get("passes") or {}).get(k) or {}).get("baseline") for k in ("ascending", "descending"))
        if not has_ref:
            _pdf_body(pdf, "No existe en esta ejecución una referencia orbital equivalente para cuantificar un cambio SAR.", size=8.0)
    fire = ancillary.get("active_fire") or {}
    if fire.get("available"):
        n = int(fire.get("combined_nominal_high_pixel_days") or 0)
        _pdf_body(pdf, "VIIRS: sin anomalías térmicas recientes." if n == 0 else f"VIIRS: {n} detecciones térmicas píxel-día recientes.", size=8.2)
    context = report.get("scene_context") or {}
    if context.get("lst_mean_c") is not None:
        _pdf_body(pdf, f"Temperatura de superficie MODIS: {float(context.get('lst_mean_c')):.1f} °C.", size=8.2)

    valid_charts = [p for p in (chart_paths or []) if p and Path(p).exists()]
    _pdf_section(pdf, "8. Evolución temporal")
    nobs = int((report.get("temporal") or {}).get("n_spectral_records") or 0)
    label = (report.get("temporal") or {}).get("range_label") or "Período seleccionado"
    if valid_charts:
        _pdf_body(pdf, f"{label}: {nobs} observaciones/compuestos válidos.", size=8.2)
        for chart in valid_charts[:4]:
            if pdf.get_y() > 215:
                pdf.add_page()
            pdf.image(chart, x=18, w=174)
            pdf.ln(3)
    else:
        _pdf_body(pdf, f"{label}: {nobs} observación(es) óptica(s) válida(s). No se dibuja una tendencia hasta disponer de suficientes fechas.", size=8.2)

    # PAGE 3 - Appendix / traceability
    pdf.add_page()
    _pdf_section(pdf, "9. Anexo de trazabilidad y metodología")
    _pdf_kv(pdf, "ID técnico", _short_id(report))
    ...
    _pdf_kv(pdf, "Resolución", f"{trace.get('comparison_scale_m')} m" if trace.get("comparison_scale_m") is not None else "N/D")
    _pdf_kv(pdf, "Regla de comparación", trace.get("comparison_rule"))
    _pdf_kv(pdf, "Escenas actual/referencia", f"{trace.get('current_n_scenes') or 'N/D'} / {trace.get('baseline_n_scenes') or 'N/D'}")

    _pdf_section(pdf, "10. Limitaciones relevantes")
    warnings = quality.get("warnings") or []
    if warnings:
        for w in warnings[:6]:
            _pdf_body(pdf, f"- {w}", size=8.0)
    else:
        _pdf_body(pdf, "No se registraron advertencias adicionales de calidad.", size=8.0)
    _pdf_body(pdf, "Los porcentajes relativos de índices normalizados pueden ser poco intuitivos cuando la referencia está cerca de cero; por eso la lectura principal del informe prioriza valores actuales, referencias y cambios absolutos.", size=8.0)

    _pdf_section(pdf, "11. Referencias metodológicas")
    for ref in report.get("references") or []:
        _pdf_body(pdf, f"[{ref.get('id')}] {ref.get('citation')}", size=7.4)
...
def render_pdf_tecnico(
    report: Dict[str, Any],
    output_path: str,
    chart_paths: Optional[List[str]] = None,
) -> str:
    """Informe técnico completo para respaldo metodológico y requerimientos externos."""
    validate_canonical_report(report)
    ...
    pdf.multi_cell(
        0, 8,
        _clean_pdf_text("INFORME TÉCNICO COMPLETO DE VIGILANCIA AMBIENTAL SATELITAL"),
        new_x="LMARGIN", new_y="NEXT",
    )
    ...
    pdf.multi_cell(
        0, 4.4,
        _clean_pdf_text(
            "Respaldo metodológico del mismo análisis canónico utilizado por BioCore Intelligence. "
            "Incluye datos, calidad, comparación temporal, evidencia complementaria, criterios configurados y referencias."
        ),
        new_x="LMARGIN", new_y="NEXT",
    )

    _pdf_section(pdf, "1. Identificación del análisis")
    _pdf_kv(pdf, "Proyecto", project.get("name"))
    _pdf_kv(pdf, "Tipo", project.get("type"))
    _pdf_kv(pdf, "Responsable", project.get("responsible"))
    _pdf_kv(pdf, "Fecha de generación", _pdf_date(report.get("generated_at")))
    _pdf_kv(pdf, "ID técnico", _short_id(report))
    _pdf_kv(pdf, "Año de referencia", project.get("baseline_year"))

    _pdf_section(pdf, "2. Síntesis técnica")
    _pdf_kv(pdf, "Estado de vigilancia", diagnosis.get("level") or "N/D")
    _pdf_body(pdf, diagnosis.get("title") or "Sin síntesis disponible.", size=8.5)
    vf = _num(quality.get("valid_fraction"))
    if vf is not None:
        _pdf_kv(pdf, "Fracción válida", f"{vf:.1%}")

    _pdf_section(pdf, "3. Fuentes, método y comparabilidad")
    _pdf_kv(pdf, "Sensor comparable", trace.get("comparison_sensor"))
    _pdf_kv(pdf, "Colección", trace.get("comparison_collection"))
    _pdf_kv(pdf, "Resolución", f"{trace.get('comparison_scale_m')} m" if trace.get("comparison_scale_m") is not None else "N/D")
    _pdf_kv(pdf, "Período actual", f"{trace.get('current_window_start') or 'N/D'} a {trace.get('current_window_end') or 'N/D'}")
    _pdf_kv(pdf, "Período de referencia", f"{trace.get('baseline_window_start') or 'N/D'} a {trace.get('baseline_window_end') or 'N/D'}")
    _pdf_kv(pdf, "Escenas actual/referencia", f"{trace.get('current_n_scenes') or 'N/D'} / {trace.get('baseline_n_scenes') or 'N/D'}")
    _pdf_kv(pdf, "Regla de comparación", trace.get("comparison_rule"))
    if trace.get("sentinel2_mask"):
        _pdf_kv(pdf, "Máscara Sentinel-2", trace.get("sentinel2_mask"))
    if trace.get("lst_qa"):
        _pdf_kv(pdf, "Control QA LST", trace.get("lst_qa"))

    _pdf_section(pdf, "4. Indicadores espectrales comparables")
    _pdf_metrics_table(pdf, report.get("metrics") or [])

    if pdf.get_y() > 210:
        pdf.add_page()

    _pdf_section(pdf, "5. Contexto de la escena reciente")
    context_rows = [
        ("Vegetación SCL", context.get("scl_vegetation_fraction"), True),
        ("Suelo desnudo SCL", context.get("scl_bare_fraction"), True),
        ("Agua SCL", context.get("scl_water_fraction"), True),
        ("Nieve/hielo SCL", context.get("scl_snow_ice_fraction"), True),
    ]
    for label, value, pct in context_rows:
        _pdf_kv(pdf, label, f"{float(value):.1%}" if value is not None and pct else "N/D")
    if context.get("lst_mean_c") is not None:
        _pdf_kv(pdf, "LST MODIS", f"{float(context.get('lst_mean_c')):.1f} °C")

    _pdf_section(pdf, "6. Hallazgos e interpretación")
    findings = diagnosis.get("findings") or []
    if not findings:
        _pdf_body(pdf, "No hay hallazgos interpretables adicionales con la evidencia disponible.", size=8.2)
    for finding in findings:
        _pdf_body(pdf, str(finding.get("object") or "Hallazgo").upper(), size=8.5, bold=True)
        for obs in finding.get("observations") or []:
            _pdf_body(pdf, f"- Observación: {obs}", size=7.8)
        for conf in finding.get("confounders") or []:
            _pdf_body(pdf, f"- Factor de interpretación: {conf}", size=7.8)
        if finding.get("interpretation"):
            _pdf_body(pdf, f"- Interpretación: {finding.get('interpretation')}", size=7.8)
        pdf.ln(1)

    _pdf_section(pdf, "7. Evidencia complementaria")
    s1 = ancillary.get("sentinel1") or {}
    if s1.get("available"):
        _pdf_body(pdf, "Sentinel-1 SAR", bold=True, size=8.4)
        for key in ("ascending", "descending"):
            ...
            if cur:
                _pdf_body(
                    pdf,
                    f"- {block.get('orbit_pass') or key.upper()}: VV actual {_fmt(cur.get('vv_db'), 2, ' dB')}; "
                    f"referencia {_fmt(base.get('vv_db'), 2, ' dB')}; cambio {_fmt(change.get('vv_db'), 2, ' dB')}; "
                    f"órbita relativa {block.get('relative_orbit') if block.get('relative_orbit') is not None else 'N/D'}.",
                    size=7.8,
                )
    else:
        _pdf_body(pdf, "Sentinel-1 SAR: sin datos complementarios válidos en esta ejecución.", size=8.0)

    fire = ancillary.get("active_fire") or {}
    if fire.get("available"):
        _pdf_body(pdf, "VIIRS Active Fire", bold=True, size=8.4)
        _pdf_body(
            pdf,
            f"- Ventana: {fire.get('window_start') or 'N/D'} a {fire.get('window_end') or 'N/D'}; "
            f"detecciones píxel-día nominales/altas: {fire.get('combined_nominal_high_pixel_days', 0)}; "
            f"alta confianza: {fire.get('combined_high_confidence_pixel_days', 0)}.",
            size=7.8,
        )

    insar = ancillary.get("insar") or {}
    if insar.get("available") or insar.get("status"):
        _pdf_body(pdf, "InSAR", bold=True, size=8.4)
        _pdf_body(pdf, str(insar.get("summary") or insar.get("status") or "Resultado InSAR disponible."), size=7.8)

    _pdf_section(pdf, "8. Serie temporal")
    nobs = int(temporal.get("n_spectral_records") or 0)
    label = temporal.get("range_label") or "Período seleccionado"
    _pdf_body(pdf, f"{label}: {nobs} observación(es)/compuesto(s) espectral(es) válido(s).", size=8.1)
    valid_charts = [p for p in (chart_paths or []) if p and Path(p).exists()]
    if valid_charts:
        for chart in valid_charts[:6]:
            if pdf.get_y() > 215:
                pdf.add_page()
            pdf.image(chart, x=18, w=174)
            pdf.ln(3)
    else:
        _pdf_body(pdf, "No se grafica una tendencia cuando el período no contiene suficientes fechas válidas.", size=8.0)

    _pdf_section(pdf, "9. Criterios específicos del proyecto")
    if regulatory.get("valid_plan"):
        _pdf_body(pdf, regulatory.get("summary") or "Criterios específicos cargados y evaluados.", size=8.2)
        for c in regulatory.get("checks") or []:
            source = c.get("source") or {}
            source_text = " | ".join(str(v) for v in [source.get("tipo"), source.get("identificador"), source.get("seccion") or source.get("pagina") or source.get("clausula")] if v)
            _pdf_body(pdf, f"- {c.get('rule_id')}: {c.get('status')} | {c.get('parameter')} | Fuente: {source_text or 'N/D'}", size=7.7)
    else:
        _pdf_body(
            pdf,
            "Los criterios regulatorios específicos del proyecto aún no están configurados en BioCore Intelligence. "
            "Esto no implica ausencia de RCA, permisos o compromisos: sólo indica que esas reglas todavía no están vinculadas al motor automático.",
            size=8.2,
        )

    _pdf_section(pdf, "10. Calidad, incertidumbre y limitaciones")
    warnings = quality.get("warnings") or []
    if warnings:
        for w in warnings:
            _pdf_body(pdf, f"- {w}", size=7.8)
    else:
        _pdf_body(pdf, "No se registraron advertencias adicionales de calidad.", size=8.0)
    _pdf_body(
        pdf,
        "Los índices espectrales y sensores remotos constituyen evidencia de vigilancia. La atribución causal, la verificación de obligaciones específicas y ciertas caracterizaciones biofísicas pueden requerir antecedentes complementarios o terreno, según el objeto evaluado.",
        size=7.8,
    )

    _pdf_section(pdf, "11. Recomendaciones")
    for rec in diagnosis.get("recommendations") or []:
        _pdf_body(pdf, f"- {rec}", size=8.0)

    _pdf_section(pdf, "12. Referencias metodológicas")
    for ref in report.get("references") or []:
        _pdf_body(pdf, f"[{ref.get('id')}] {ref.get('citation')} Uso: {ref.get('role') or ''}", size=7.2)

    _pdf_section(pdf, "13. Alcance de uso")
    _pdf_body(
        pdf,
        "Este informe técnico documenta un análisis de vigilancia ambiental satelital. Su uso en una presentación formal ante SEA, SMA, DGA u otra autoridad debe ajustarse al procedimiento, instrumento ambiental y requerimiento específico aplicable al proyecto.",
        size=8.0,
    )
...
def render_analysis_header(report: Dict[str, Any]):
    ...
    st.markdown(
        ...
        <div class="bc-v2-hero">
            <div class="bc-v2-eyebrow">BioCore Intelligence · último análisis</div>
            <div class="bc-v2-title">{project.get('name') or 'Proyecto sin nombre'}</div>
            <div class="bc-v2-sub">{project.get('type') or 'N/D'} · referencia {project.get('baseline_year') or 'N/D'} · {generated_at}</div>
        </div>
        """, unsafe_allow_html=True)
    with st.expander("Detalles de trazabilidad", expanded=False):
        st.caption(f"ID de análisis: {aid}")
        st.caption(f"Versión: {report.get('schema_version') or 'N/D'} · {report.get('report_method_version') or 'N/D'}")

def render_status_row(report: Dict[str, Any]):
    quality = report.get("quality") or {}
    diagnosis = report.get("diagnosis") or {}
    regulatory = report.get("regulatory") or {}
    quality_status = "Utilizable" if quality.get("usable") is True else "Insuficiente"
    technical_raw = str(diagnosis.get("level") or "No evaluado")
    regulatory_raw = str(regulatory.get("status") or "Sin criterio cargado")
    technical_status = {
        "SIN SEÑAL PRIORITARIA": "Sin señal prioritaria",
        "SEÑAL A REVISAR": "Señal a revisar",
        "VERIFICACIÓN PRIORITARIA": "Verificación prioritaria",
        "EVIDENCIA INSUFICIENTE": "Evidencia insuficiente",
    }.get(technical_raw.upper(), technical_raw.title())
    regulatory_status = {
        "SIN CRITERIO REGULATORIO CARGADO": "No cargados",
        "DENTRO DEL CRITERIO CONFIGURADO": "Dentro del criterio",
        "UMBRAL INTERMEDIO ALCANZADO": "Umbral intermedio",
        "UMBRAL LIMITE ALCANZADO": "Umbral límite",
        "NO EVALUABLE": "No evaluable",
    }.get(regulatory_raw.upper(), regulatory_raw.title())
    st.html(f"""
    <style>
      .bc-status-grid{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin:10px 0 16px}}
      .bc-status-card{{border:1px solid rgba(148,163,184,.18);border-radius:14px;padding:14px;background:rgba(15,23,42,.38)}}
      .bc-status-label{{color:#94a3b8;font-size:.72rem;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px}}
      .bc-status-value{{color:#f8fafc;font-size:1rem;font-weight:700;line-height:1.25}}
      @media(max-width:700px){{.bc-status-grid{{grid-template-columns:1fr}}}}
    </style>
    <div class="bc-status-grid">
      <div class="bc-status-card"><div class="bc-status-label">Calidad de datos</div><div class="bc-status-value">{quality_status}</div></div>
      <div class="bc-status-card"><div class="bc-status-label">Estado de vigilancia</div><div class="bc-status-value">{technical_status}</div></div>
      <div class="bc-status-card"><div class="bc-status-label">Criterios del proyecto</div><div class="bc-status-value">{regulatory_status}</div></div>
    </div>
    """)
...
def render_executive_summary(report: Dict[str, Any]):
    ...
    if regulatory.get("valid_plan") and regulatory.get("summary"):
        st.markdown(
            f"""
            <div class="bc-v2-note">
            <strong>Criterios específicos del proyecto:</strong>
    ...
    st.caption(
        "BioCore Intelligence conserva la evidencia, la comparación y la trazabilidad para que cada resultado pueda revisarse y repetirse."
    )
    ...
    with t_insar:
        if not insar.get("available"):
            st.info(
                ...
                "BioCore Intelligence no sustituye fase SLC con Sentinel-1 GRD."
            )
...
def render_temporal(
    ...
    if charts:
        ...
        st.info(
            ...
            "BioCore Intelligence no completa la serie con puntos ficticios."
        )
...
def render_regulatory(report: Dict[str, Any]):
    ...
    st.subheader("Criterios específicos del proyecto")

    if not regulatory:
        st.info(
            "Las obligaciones y umbrales específicos del proyecto todavía no están cargados en BioCore Intelligence. "
            "Esto no significa que el proyecto no tenga una RCA o permisos; sólo que esas reglas aún no se han incorporado a la plataforma."
        )
        return
...
def _store_analysis(prefix: str, report, temporal, charts, project_data):
    ...
    st.session_state[f"{prefix}_project_data"] = project_data
...
def _render_stored_analysis(prefix: str):
    ...
    render_status_row(report)
    render_executive_summary(report)
    warnings = list((report.get("quality") or {}).get("warnings") or [])
    warnings += [w for w in (temporal.get("warnings") or []) if w not in warnings]
    if warnings:
        st.warning("\n\n".join(f"• {w}" for w in warnings[:4]))
    st.info("El PDF y el envío manual por Telegram están en la sección **Informes**.")
    ...
    if st.button("🧹 Cerrar resultado", key=f"clear_{prefix}", use_container_width=True):
        for suffix in ("report", "temporal", "charts", "project_data"):
            st.session_state.pop(f"{prefix}_{suffix}", None)
        st.rerun()

def _latest_canonical_for_project(project_name: str):
    try:
        rows = (supabase.table("historial_reportes")
                .select("payload_json,created_at,analysis_id")
                .eq("proyecto", project_name)
                .order("created_at", desc=True).limit(20).execute().data or [])
        for row in rows:
            payload = row.get("payload_json")
            if isinstance(payload, dict) and verify_analysis_id(payload):
                return payload
    except Exception:
        return None
    return None

def _report_for_project(project_name: str):
    current = st.session_state.get("vig_report")
    if isinstance(current, dict) and (current.get("project") or {}).get("name") == project_name:
        return current, st.session_state.get("vig_charts") or []
    return _latest_canonical_for_project(project_name), []

def render_report_center(project_data: Dict[str, Any], report: Dict[str, Any], chart_paths=None):
    if not report:
        st.info("Aún no hay un análisis disponible. Ve a **Vigila**, ejecuta el análisis y vuelve aquí.")
        return
    ...
    st.subheader(project.get("name") or "Informe")
    st.caption(f"Último análisis · {generated_at} · {diagnosis.get('level') or 'N/D'}")

    tipo = st.radio(
        "¿Qué necesitas entregar?",
        ["Informe ejecutivo", "Informe técnico completo"],
        horizontal=False,
        key=f"report_type_{project.get('name')}",
        help="Ambos se generan desde el mismo análisis y contienen los mismos valores.",
    )

    if tipo == "Informe ejecutivo":
        st.info(
            "**Para cliente y toma de decisiones.** Resume hallazgos, cambios y próximos pasos en lenguaje claro; la metodología queda al final."
        )
        prefix = "Ejecutivo"
        renderer = render_pdf
    else:
        st.info(
            "**Para respaldo metodológico o requerimientos externos.** Incluye sensores, QA, comparabilidad, índices, evidencia complementaria, limitaciones y referencias."
        )
        prefix = "Tecnico"
        renderer = render_pdf_tecnico

    try:
        filename = f"BioCore_{prefix}_{str(project.get('name') or 'Proyecto').replace(' ','_')}_{str(report.get('analysis_id') or '')[:12]}.pdf"
        path = os.path.join(tempfile.gettempdir(), filename)
        renderer(report, path, chart_paths=chart_paths or [])
        st.download_button(
            f"📄 Descargar {tipo.lower()}",
        ...
        st.error(f"No fue posible preparar el PDF: {exc}")

    st.markdown("#### Telegram")
    st.caption("Resumen breve para avisar al cliente; el detalle queda en los informes PDF.")
    if st.button("📡 Enviar resumen por Telegram", key=f"report_send_{project.get('name')}", use_container_width=True):
    ...
    with st.expander("Vista previa del mensaje de Telegram", expanded=False):
        st.text(format_telegram(report))
...
if st.session_state.get("admin_mode"):
    sections = ["🛰️ Vigila", "📄 Informes", "🗃️ Base de datos", "👥 Clientes", "📖 Guía"]
else:
    sections = ["🛰️ Vigila", "📄 Informes", "📨 Historial", "⚙️ Telegram", "📖 Guía"]
section = st.selectbox("Sección", sections, key="main_section", label_visibility="collapsed")

if section == "🛰️ Vigila":
    st.title("🛰️ Vigilancia ambiental")
    st.caption("Observa el territorio y ejecuta un análisis. Los detalles técnicos quedan plegados para mantener la pantalla clara.")
    if not projects:
        ...
        try:
            coords=obtener_coordenadas_correctamente(project_data)
            view_mode=st.radio("Vista", ["🛰️ Mapa 2D","🗻 Terreno 3D"], horizontal=True, key="vig_map_mode")
            if view_mode=="🛰️ Mapa 2D":
                folium_static(dibujar_mapa_biocore(coords), width=850, height=460)
            else:
                render_mapa_3d_biocore(coords, osm_context=None, height=470)
                st.caption("Relieve Copernicus DEM GLO-30.")
        except Exception as exc:
            st.error(f"No se pudo representar el proyecto: {exc}")
        range_label=st.selectbox("Ventana de vigilancia", ["Últimos 7 días","Últimas 2 semanas","Último mes","Últimos 3 meses"], index=2, key="vig_range")
        if st.button("🚀 Analizar", key="vig_run", use_container_width=True):
            with st.spinner("Consultando sensores y construyendo el análisis..."):
                try:
                    report, temporal, charts=ejecutar_analisis_canonico(project_data, range_label, save=True)
                    ...
                    if save_warning: st.warning(save_warning)
                except Exception as exc:
    ...
    st.title("📄 Informes")
    st.caption("Elige un informe ejecutivo para el cliente o un informe técnico completo para respaldo metodológico y requerimientos externos.")
    if not projects:
        st.warning("No hay proyectos disponibles.")
    else:
        names=[p.get("Proyecto") for p in projects]
        selected=st.selectbox("Proyecto", names, key="report_project")
        project_data=next(p for p in projects if p.get("Proyecto")==selected)
        report, charts=_report_for_project(selected)
        render_report_center(project_data, report, charts)

elif section == "🗃️ Base de datos" and st.session_state.get("admin_mode"):
    st.title("🗃️ Base de datos")
    db_view=st.radio("Ver", ["Usuarios","Reportes"], horizontal=True, key="db_view")
    if db_view=="Usuarios":
        try:
            rows=supabase.table("usuarios").select("*").execute().data or []
            safe=[]
            for r in rows:
                rr=dict(r); rr.pop("password_cliente",None); safe.append(rr)
            if safe: st.dataframe(pd.DataFrame(safe), use_container_width=True, hide_index=True)
            else: st.info("No hay usuarios registrados.")
        except Exception as exc: st.error(f"Error cargando usuarios: {exc}")
    else:
        try:
            rows=(supabase.table("historial_reportes").select("proyecto,tipo,created_at,analysis_id,estado,source_data_date").order("created_at",desc=True).limit(200).execute().data or [])
            display=[{"Proyecto":r.get("proyecto"),"Fecha":str(r.get("created_at") or "")[:16].replace("T"," "),"Estado":r.get("estado"),"Datos":r.get("source_data_date"),"ID":str(r.get("analysis_id") or "")[:12]} for r in rows]
            if display: st.dataframe(pd.DataFrame(display), use_container_width=True, hide_index=True)
            else: st.info("No hay reportes guardados.")
        except Exception as exc: st.error(f"Error cargando reportes: {exc}")

elif section == "👥 Clientes" and st.session_state.get("admin_mode"):
    st.title("👥 Clientes")
    action=st.radio("Acción", ["Nuevo","Editar","Eliminar"], horizontal=True, key="client_action")
    if action=="Nuevo":
        with st.form("new_client_v3"):
            ...
            email=st.text_input("Correo"); telegram_id=st.text_input("ID Telegram"); password=st.text_input("Contraseña *", type="password")
            submitted=st.form_submit_button("Registrar", use_container_width=True)
        if submitted:
            try:
                if not proyecto or not coords_text or not password: raise ValueError("Completa los campos obligatorios.")
                coords=limpiar_coordenadas(json.loads(coords_text))
                record={"Proyecto":proyecto.strip(),"Tipo":tipo,"Coordenadas":json.dumps(coords),"email_cliente":email.strip().lower() or None,"password_cliente":hash_password(password),"id_telegram":telegram_id.strip() or None,"ano_linea_base":int(baseline)}
                supabase.table("usuarios").insert(record).execute(); st.success("Cliente registrado."); st.rerun()
            except Exception as exc: st.error(f"No fue posible registrar: {exc}")
    elif action=="Editar":
        all_names=[p.get("Proyecto") for p in _accessible_projects()]
        if not all_names: st.info("No hay clientes registrados.")
        else:
            selected_edit=st.selectbox("Cliente",all_names,key="edit_client_name_v3"); current=_project_by_name(selected_edit) or {}
            with st.form("edit_client_v3"):
                tipos=["MINERIA","GLACIAR","BOSQUE","HUMEDAL","AGRICOLA"]; current_type=current.get("Tipo") if current.get("Tipo") in tipos else tipos[0]
                tipo=st.selectbox("Tipo",tipos,index=tipos.index(current_type)); raw=current.get("Coordenadas"); raw=raw if isinstance(raw,str) else json.dumps(raw or [])
                coords_text=st.text_area("Coordenadas JSON",value=raw); baseline=st.number_input("Año de línea base",min_value=2000,max_value=datetime.now().year,value=int(current.get("ano_linea_base") or 2015))
                email=st.text_input("Correo",value=current.get("email_cliente") or ""); telegram_id=st.text_input("ID Telegram",value=str(current.get("id_telegram") or "")); password=st.text_input("Nueva contraseña (opcional)",type="password")
                submitted=st.form_submit_button("Guardar cambios",use_container_width=True)
            if submitted:
                try:
                    coords=limpiar_coordenadas(json.loads(coords_text)); update={"Tipo":tipo,"Coordenadas":json.dumps(coords),"ano_linea_base":int(baseline),"email_cliente":email.strip().lower() or None,"id_telegram":telegram_id.strip() or None}
                    if password.strip(): update["password_cliente"]=hash_password(password)
                    supabase.table("usuarios").update(update).eq("Proyecto",selected_edit).execute(); st.success("Cliente actualizado."); st.rerun()
                except Exception as exc: st.error(f"No fue posible actualizar: {exc}")
    else:
        all_names=[p.get("Proyecto") for p in _accessible_projects()]
        if not all_names: st.info("No hay clientes registrados.")
        else:
            selected_del=st.selectbox("Cliente",all_names,key="delete_client_name_v3"); confirm=st.checkbox(f"Confirmo eliminar {selected_del}")
            if st.button("Eliminar cliente",disabled=not confirm,use_container_width=True):
                try: supabase.table("usuarios").delete().eq("Proyecto",selected_del).execute(); st.success("Cliente eliminado."); st.rerun()
                except Exception as exc: st.error(f"No fue posible eliminar: {exc}")

elif section == "📨 Historial" and not st.session_state.get("admin_mode"):
    st.title("📨 Historial")
    project_name=st.session_state.get("proyecto_cliente")
    try:
        rows=(supabase.table("historial_reportes").select("created_at,source_data_date,analysis_id,estado,payload_json").eq("proyecto",project_name).order("created_at",desc=True).limit(100).execute().data or [])
    except Exception as exc:
        rows=[]; st.error(f"No fue posible cargar el historial: {exc}")
    if not rows: st.info("Aún no hay análisis guardados.")
    else:
        display=[{"Fecha":str(r.get("created_at") or "")[:16].replace("T"," "),"Datos":r.get("source_data_date"),"Estado":r.get("estado"),"ID":str(r.get("analysis_id") or "")[:12]} for r in rows]
        st.dataframe(pd.DataFrame(display),use_container_width=True,hide_index=True)
        canonical=[r for r in rows if isinstance(r.get("payload_json"),dict)]
        if canonical:
            options={f"{str(r.get('created_at') or '')[:16].replace('T',' ')} · {str(r.get('analysis_id') or '')[:12]}":r for r in canonical}
            label=st.selectbox("Abrir análisis",list(options.keys())); selected_report=options[label]["payload_json"]
            if verify_analysis_id(selected_report):
                render_analysis_header(selected_report); render_status_row(selected_report); render_executive_summary(selected_report)
                with st.expander("Detalles técnicos",expanded=False): render_primary_metrics(selected_report); render_regulatory(selected_report)
            else: st.error("El registro seleccionado no supera la verificación de integridad.")

elif section == "⚙️ Telegram" and not st.session_state.get("admin_mode"):
    st.title("⚙️ Telegram")
    st.caption("Configura aquí los reportes automáticos. El envío manual del último informe está en Informes.")
    mostrar_formulario_reportes(); st.markdown("---"); mostrar_resumen_reportes()
