...
def render_status_row(report: Dict[str, Any]):
    ...
    regulatory_status = {
        "SIN CRITERIO REGULATORIO CARGADO": "Sin criterio cargado",
    ...
    <div class="bc-status-grid">
      <div class="bc-status-card"><div class="bc-status-label">Calidad de evidencia</div><div class="bc-status-value">{quality_status}</div></div>
      <div class="bc-status-card"><div class="bc-status-label">Estado técnico</div><div class="bc-status-value">{technical_status}</div></div>
      <div class="bc-status-card"><div class="bc-status-label">Estado regulatorio</div><div class="bc-status-value">{regulatory_status}</div></div>
    </div>
    """)
...
def render_report_center(project_data: Dict[str, Any], report: Dict[str, Any], chart_paths=None):
    ...
    with c1:
        try:
            filename = f"BioCore_{str(project.get('name') or 'Proyecto').replace(' ','_')}_{str(report.get('analysis_id') or '')[:12]}.pdf"
            path = os.path.join(tempfile.gettempdir(), filename)
            render_pdf(report, path, chart_paths=chart_paths or [])
            st.download_button("📄 Descargar PDF", data=Path(path).read_bytes(), file_name=filename, mime="application/pdf", use_container_width=True)
        except Exception as exc:
            st.error(f"No fue posible preparar el PDF: {exc}")
    with c2:
        if st.button("📡 Enviar por Telegram", key=f"report_send_{project.get('name')}", use_container_width=True):
...
if section == "🛰️ Vigila":
    ...
    st.title("📄 Informes")
    st.caption("Descarga o envía el último análisis. Aquí no se repiten todos los índices de Vigila.")
    if not projects:
