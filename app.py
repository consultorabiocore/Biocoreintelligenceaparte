...
def crear_portada_biocore():
    # st.html evita que el HTML sea interpretado como un bloque de código Markdown
    # en móviles, que era lo que ocurría con la portada anterior.
    st.html("""
    <style>
    .bc-public{
        max-width:1040px;
    ...
    .bc-hero2{
        ...
        background:
          radial-gradient(circle at 92% 8%,rgba(14,165,233,.14),transparent 33%),
          radial-gradient(circle at 5% 100%,rgba(34,197,94,.09),transparent 32%),
          linear-gradient(145deg,#081426,#0b1220 60%,#07111d);
    ...
    .bc-problem strong{color:#f8fafc;}
    .bc-solution{
        margin-top:18px;
        max-width:850px;
        color:#94a3b8;
    ...
    .bc-promise-card span{
        color:#94a3b8;
        font-size:.79rem;
        line-height:1.5;
    }
    .bc-block{margin-top:28px;}
    .bc-block h2{
        color:#f1f5f9;
    ...
    .bc-cta2 span{color:#94a3b8;font-size:.80rem;line-height:1.5;}
    .bc-cta2 a{color:#7dd3fc;text-decoration:none;}
    @media(max-width:720px){
        .bc-public{width:100%;overflow:hidden;}
        .bc-hero2{border-radius:17px;padding:23px 17px;}
        .bc-promise{grid-template-columns:1fr;}
        .bc-flow2{grid-template-columns:1fr 1fr;}
    }
    @media(max-width:420px){
        .bc-flow2{grid-template-columns:1fr;}
        .bc-hero2 h1{font-size:2.18rem;}
    }
    </style>

    <div class="bc-public">
      <section class="bc-hero2">
        <div class="bc-brand">BioCore Intelligence</div>

        <h1>Detecta cambios ambientales antes de que se conviertan en un problema.</h1>

        <p class="bc-problem">
          <strong>Cuando una desviación se descubre tarde, queda menos tiempo para
          verificarla, corregirla y documentar la respuesta.</strong>
          Eso puede traducirse en mayores costos, observaciones regulatorias y una
          exposición innecesaria a incumplimientos o sanciones.
        </p>

        <p class="bc-solution">
          BioCore Intelligence vigila el territorio con datos satelitales, radar,
          series temporales y análisis 3D para detectar señales tempranas,
          explicar la evidencia y dejar un historial verificable antes de tomar
          decisiones.
        </p>

        <div class="bc-pills">
          <span class="bc-pill">Detección temprana</span>
          <span class="bc-pill">Vigilancia satelital</span>
          <span class="bc-pill">Terreno 3D</span>
          <span class="bc-pill">Radar e InSAR</span>
          <span class="bc-pill">Alertas trazables</span>
          <span class="bc-pill">Informes verificables</span>
        </div>

        <div class="bc-promise">
          <div class="bc-promise-card">
            <b>Anticipa</b>
            <span>Identifica señales y cambios antes de depender únicamente de una inspección tardía.</span>
          </div>
          <div class="bc-promise-card">
            <b>Comprende</b>
            <span>Distingue dato, cambio, interpretación, confusores y límites de la evidencia.</span>
          </div>
          <div class="bc-promise-card">
            <b>Documenta</b>
            <span>Conserva metodología, fechas, sensores, resultados y reportes vinculados al mismo análisis.</span>
          </div>
        </div>
      </section>

      <section class="bc-block">
        <h2>Así funciona BioCore</h2>
        <div class="bc-block-intro">
          Del territorio a una evidencia que pueda revisarse y explicarse.
        </div>
        <div class="bc-flow2">
          <div class="bc-flow-card">
            <div class="bc-flow-n">01 · OBSERVA</div>
            <b>Territorio y sensores</b>
            <span>Área de estudio, satélites, radar, clima y relieve.</span>
          </div>
          <div class="bc-flow-card">
            <div class="bc-flow-n">02 · COMPARA</div>
            <b>Cambio temporal</b>
            <span>Referencia comparable, calidad de datos y evolución real.</span>
          </div>
          <div class="bc-flow-card">
            <div class="bc-flow-n">03 · INTERPRETA</div>
            <b>Evidencia explicable</b>
            <span>Qué cambió, qué no puede concluirse y qué debe verificarse.</span>
          </div>
          <div class="bc-flow-card">
            <div class="bc-flow-n">04 · DOCUMENTA</div>
            <b>Reporte trazable</b>
            <span>PDF, historial, alertas y criterios específicos del proyecto.</span>
          </div>
        </div>
      </section>

      <div class="bc-diff">
        <b>No es un “semáforo” automático.</b>
        BioCore no convierte un índice satelital en una sentencia. La plataforma
        conserva la evidencia, explicita sus límites y sólo aplica criterios
        regulatorios cuando existe una fuente trazable del proyecto.
      </div>

      <section class="bc-block">
        <h2>Para proyectos donde detectar tarde cuesta más</h2>
        <div class="bc-audience2">
          <span>⛏️ Minería</span>
          <span>🌿 Gestión ambiental</span>
          <span>🏗️ Infraestructura</span>
          <span>🗺️ Gestión territorial</span>
      ...
      <div class="bc-cta2">
        <b>Menos incertidumbre. Más tiempo para actuar y documentar.</b>
        <span>
          Clientes: utiliza <strong>Acceso de clientes</strong> desde el panel lateral.<br>
    ...
    # Vista 3D demostrativa pública.
    # No usa HTML ni servicios externos, por lo que se renderiza de forma estable
    # en Streamlit móvil. Los clientes ven el DEM real de su proyecto al ingresar.
    x = np.linspace(-3.2, 3.2, 46)
    y = np.linspace(-3.0, 3.0, 42)
    xx, yy = np.meshgrid(x, y)
    zz = (
        1050
        + 380 * np.exp(-((xx + 1.0) ** 2 + (yy - 0.4) ** 2) / 2.1)
        + 250 * np.exp(-((xx - 1.4) ** 2 + (yy + 1.0) ** 2) / 1.3)
        - 120 * np.exp(-((xx - 0.2) ** 2 + (yy - 1.1) ** 2) / 0.8)
        + 35 * np.sin(xx * 1.4) * np.cos(yy * 1.1)
    )
    ...
    fig_demo = go.Figure(
        data=[
            go.Surface(
                ...
                colorscale="Earth",
                showscale=False,
                hovertemplate="Relieve demostrativo<extra>BioCore 3D</extra>",
            )
        ]
    )
    fig_demo.add_trace(
        go.Scatter3d(
            x=[-1.6, -0.4, 1.4, 1.8, 0.5, -1.6],
            y=[-0.9, 1.5, 1.1, -0.7, -1.6, -0.9],
            z=[1390, 1310, 1250, 1200, 1160, 1390],
            mode="lines",
            line=dict(width=8),
            name="Área de estudio",
            hovertemplate="Área de estudio<extra></extra>",
        )
    )
    fig_demo.update_layout(
        title=dict(
            text="Vista 3D · BioCore Intelligence",
            x=0.02,
            xanchor="left",
            font=dict(size=16),
        ),
        ...
        scene=dict(
            ...
            camera=dict(eye=dict(x=1.35, y=-1.55, z=1.05)),
            aspectmode="data",
        ),
        legend=dict(
            orientation="h",
    ...
    st.plotly_chart(
        ...
        key="biocore_public_3d_demo",
    )
    st.caption(
        "Vista demostrativa. Al iniciar sesión, el visor 3D utiliza el área real "
        "del proyecto y Copernicus DEM GLO-30."
    )
